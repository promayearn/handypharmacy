...
import cstu.handypharmacy.controller.GetUsers;
...
public class UserDetail extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        GetUsers.getUsersData(new GetUsers.DataCallback() {
            @Override
            public void onFinish() {
                Log.e(TAG, "users size " + User.useres.size());
                for (int i = 0; i < User.useres.size(); i++) {
                    if (User.useres.get(i).getName().equals(userName)) {
                        Log.e(TAG, "Test " + User.useres.get(i).getEmail());
                        final int finalI = i;
                        GetGeneralUsers.getGeneralUsersData(new GetGeneralUsers.DataCallback() {
                            @Override
                            public void onFinish() {
                                for (int j = 0; j < Member.members.size(); j++) {
                                    if (Member.members.get(j).getEtEmail().equals(User.useres.get(finalI).getEmail())) {
                        ...
                        GetPharmacist.getPharmacistData(new GetPharmacist.DataCallback() {
                            ...
                            public void onFinish() {
                                for (int k = 0; k < Pharmacist.pharmacists.size(); k++) {
                                    if (Pharmacist.pharmacists.get(k).getEmail().equals(User.useres.get(finalI).getEmail())) {
